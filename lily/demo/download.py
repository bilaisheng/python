...
def download5(url, proxy=None, user_agent='test', num_retries=2, data=None):
    ...
    # 异常处理，捕获异常
        ...
        if num_retries > 0:
            ...
            if hasattr(e, 'code') and 500 <= e.code < 600:
                # 调用自身
                html5 = download5(url, proxy, user_agent, num_retries-1)
    return html5
...
html = download4(url2, user_agent='"360":"User-Agent: Mozilla/4.0 (compatible; MSIE 7.0;Window NT 5.1;360SE)"')

# 下载百度首页，设置用户代理为UC浏览器
html1 = download4(url2, user_agent='"UC":"User-Agent:UCWEB7.0.2.37/28/999"')

# 打开pc.html,如路径中无此文件会自动创建
myfile = open('pc.html', "w+", encoding="utf-8")

# 打开mb.html,如果路径中无此文件会自动创建
myfile1 = open("mb.html", "w+", encoding="utf-8")

# 将下载到的内容写入pc.html(使用360代理)
myfile.write(html.decode("utf-8"))

# 将下载到的内容写入mb.html（使用UC代理）
myfile1.write(html1.decode("utf-8"))
